...
/* We wnat to find out the most popular music genre for each country.
We determine the most popular genre as the genre with the highest amount of puchases.
write a query that returns each country along with the top genre.
For countries where the maximum numhber of purchases is shared return all genres.*/
...
with recursive
     customer_with_country as (select customer.customer_id,first_name,last_name,billing_country,sum(total) as total_spending
							   from invoice
							   join customer on customer.customer_id=invoice.customer_id
							   group by 1,2,3,4
							   order by 2,3 desc),
